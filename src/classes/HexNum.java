...
//    }
    ...
    private static String processString(String string) {
        ...
        // Add a leading zero if string is of odd length and convert to uppercase
    ...
    // √a (% p) ≡ a ^ ((p + 1) / 4) (% p)
    public HexNum[] sqrt() {
        if (!modulus.mod(FOUR).equals(THREE)) {
            throw new ArithmeticException("Square roots not supported for modulus: " + modulus.toString());
        }
        HexNum root1 =
                new HexNum(this.scalar.modPow(modulus.add(ONE).divide(FOUR), modulus));
    ...
    public boolean equals(HexNum other) {
