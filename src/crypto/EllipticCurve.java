...
//        public final BigInteger INT_TWO = new BigInteger("2");
        ...
        public Point pointDouble() {
            ...
            if (this.infinity || this.y.equals(HexNum.HEX_ZERO)) {
                return new Point();
            }
            // Get slope
            HexNum slope = HexNum.HEX_THREE.mul(this.x.pow(HexNum.HEX_TWO)).div(HexNum.HEX_TWO.mul(this.y));     // λ = (3 * (x ^ 2)) / (2 * y) (% p)
            // Get x coordinate
            HexNum Rx = slope.pow(HexNum.HEX_TWO).sub(HexNum.HEX_TWO.mul(this.x));                    // Rx = (λ ^ 2) - (2 * x) (% p)
            // Get y coordinate
        ...
        public Point pointAdd(Point Q) {
            ...
            // Get x coordinate
            HexNum Rx = slope.pow(HexNum.HEX_TWO).sub(this.x).sub(Q.x);         // Rx = (λ ^ 2) - x - Gx (% p)
            // Get y coordinate
        ...
        public Point pointMultiply(HexNum k) {
            ...
            while (n.compareTo(HexNum.BIG_ZERO) > 0) {
                bitString = (n.mod(HexNum.BIG_TWO).compareTo(HexNum.BIG_ZERO) == 0 ? "0" : "1") + bitString;
                n = n.divide(HexNum.BIG_TWO);
            }
...
//    public static final HexNum THREE = new HexNum("03");
    ...
    public static final String SECP256K1_a = "00";
    public static final String SECP256K1_b = "07";
    ...
    public HexNum computePublicKey(HexNum privateKey) {
        Point publicPoint = G.pointMultiply(privateKey);
        return publicPoint.x.concat(publicPoint.y);
    }
